...
APP_NAME = "Video2Text Desktop"
WINDOW_SIZE = "1180x760"


class Video2TextApp(ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        header = ctk.CTkFrame(self, fg_color="#020403", corner_radius=0)
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="> aplicación local de escritorio · descarga video/audio · transcribe en tu PC · exporta TXT y Word",
            font=ctk.CTkFont(family="Consolas", size=13),
            text_color="#b8ffd9",
        )
        subtitle.grid(row=1, column=0, sticky="w", padx=24, pady=(0, 16))

        sidebar = ctk.CTkFrame(self, width=330, fg_color="#03140c", border_color="#00ff88", border_width=1)
        sidebar.grid(row=1, column=0, sticky="nsw", padx=16, pady=16)
        ...
        self.file_label.pack(anchor="w", padx=18, pady=(0, 8))
        ...
        self.load_file_button.pack(fill="x", padx=18, pady=8)
        ...
        self.process_button.pack(fill="x", padx=18, pady=(18, 10))
        ...
        self.download_txt_button.pack(fill="x", padx=18, pady=6)
        ...
        self.download_word_button.pack(fill="x", padx=18, pady=6)
        ...
        self.status_label.pack(anchor="w", padx=18, pady=(18, 8))

        main_panel = ctk.CTkFrame(self, fg_color="#020403", border_color="#00ff88", border_width=1)
        main_panel.grid(row=1, column=1, sticky="nsew", padx=(0, 16), pady=16)
    ...
    def _option(self, parent: ctk.CTkFrame, label: str, values: list[str], default: str) -> ctk.CTkOptionMenu:
        ctk.CTkLabel(
            parent,
            text=label,
            font=ctk.CTkFont(family="Consolas", size=13),
        ...
        option.set(default)
        option.pack(fill="x", padx=18, pady=(0, 4))
        return option
    ...
    def _process_worker(self) -> None:
        try:
            ...
            self.log_queue.put(("log", "[SCAN] Analizando audio..."))
            ...
            transcript = "\n".join(parts).strip() + "\n"
            txt_path = base_output / "transcript.txt"
            json_path = base_output / "segments.json"
            srt_path = base_output / "subtitles.srt"
            docx_path = base_output / "transcript.docx"
            ...
            self.log_queue.put(("done", {"transcript": transcript, "docx": docx_bytes, "output": base_output}))
        ...
        try:
            while True:
                ...
                if msg_type == "log":
                    ...
                    self._log("")
                    self._log("[DONE] Archivos generados. Ya puedes descargar TXT o Word.")
                elif msg_type == "error":
                    self._set_busy(False)
    ...
    def _srt_stamp(seconds: float) -> str:
        return Video2TextApp._stamp(seconds).replace(".", ",")
    ...
    def _write_srt(cls, segments: list[dict[str, Any]], path: Path) -> None:
        ...
        for index, segment in enumerate(segments, start=1):
            lines.extend([
                str(index),
                f"{cls._srt_stamp(segment['start'])} --> {cls._srt_stamp(segment['end'])}",
